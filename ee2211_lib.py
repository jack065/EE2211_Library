...
def _ensure_2d(arr):
    """Helper to ensure arrays are 2D for matrix operations."""
    arr = np.array(arr)
    if arr.ndim == 1:
...
def polynomial_regression(x, y, degree, x_pred=None, lmbda=0.0, pearson=False):    
    ...
    if pearson:
        for i in range(y.shape[1]):
            # Use np.corrcoef properly handling edge cases of zero variance
            if np.std(y[:, i]) == 0 or np.std(y_test[:, i]) == 0:
                print(f"Pearson correlation coefficient for output dimension {i}: Undefined (zero variance)\n")
            else:
...
def one_hot_encoder(x, y, degree = 1, x_pred = None, lmbda = 0):
    ...
    # BUG FIX: Evaluate X_poly.shape, not x.shape
    if X_poly.shape[0] < X_poly.shape[1]:
        try:
...
def pearson_correlation_rows(X, Y):
    ...
    # Handle all-NaN case safely
    if np.all(np.isnan(corrs)):
        print("All rows have zero variance.")
        return corrs, None, None, None

    best_idx = int(np.nanargmax(np.abs(corrs)))
    best_feature_number = best_idx + 1  
...
def regression_tree(x, y, initial_threshold=None, max_depth=3):
    ...
    def build_tree(x_subset, y_subset, indices, depth, threshold):
        ...
        node = {'depth': depth, 'n_samples': len(y_subset), 'prediction': y_pred, 'mse': mse, 'indices': indices}
        ...
        node['left'] = build_tree(x_subset[left_mask], y_subset[left_mask], indices[left_mask], depth + 1, left_threshold)
        node['right'] = build_tree(x_subset[right_mask], y_subset[right_mask], indices[right_mask], depth + 1, right_threshold)
    ...
    def collect_mse_by_depth(node, depth_mse_dict):
        ...
        if 'left' in node: collect_mse_by_depth(node['left'], depth_mse_dict)
        if 'right' in node: collect_mse_by_depth(node['right'], depth_mse_dict)
